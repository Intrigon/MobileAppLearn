...
public class ALoader extends AppCompatActivity  implements LoaderManager.LoaderCallbacks<Void> {
    ...
    static class ContohLoader extends AsyncTaskLoader<Void> {
        static WeakReference<ALoader> mActivity;
        int mCounter = 0;

        public ContohLoader(@NonNull Context context, int n, ALoader main) {
            super(context);
            mCounter = n;
            mActivity = new WeakReference<ALoader>(main);
        }
        @Nullable
        @Override
